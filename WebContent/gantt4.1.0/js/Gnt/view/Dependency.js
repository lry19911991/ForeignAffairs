...
Ext.define("Gnt.view.Dependency", {
                ...
                '<tpl for=".">' +
                    (
                        '<tpl for="lineCoordinates">' +
                            ...
                            '{[values.x1!=values.x2 ? "width:" + (' + this.lineWidth + '+ Math.abs(values.x1-values.x2)) + "px": ""]};' +
                            '{[values.y1!=values.y2 ? "height:" + (' + this.lineWidth + '+ Math.abs(values.y1-values.y2)) + "px": ""]}">' +
                            '</div>' +
                        '</tpl>' +
    ...
    createDependencyCanvas : function(nodeContainerEl) {
        ...
        return Ext.fly(nodeContainerEl).insertFirst({
            cls : 'sch-dependencyview-ct ' + (this.lineWidth === 1 ? ' sch-dependencyview-thin ' : 'sch-dependencyview-thick')
        });
    },

    getDependencyCanvas : function() {
        var me = this,
            nodeContainer = me.ganttView.getNodeContainer(),
    ...
    updateDependencies: function (tasks) {
        ...
        Ext.Array.each(tasks, function (task) {
            Ext.Array.each(task.getAllDependencies(), function (dependency) {
