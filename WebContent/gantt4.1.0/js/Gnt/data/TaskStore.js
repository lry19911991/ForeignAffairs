...
Ext.define('Gnt.data.TaskStore', {
    ...
    */
    ...
    fillSingleTaskWithDepInfo : function(task, onLinkEstablished) {
        var me = this,
        ...
        if (dependencyStore) {
            ...
            dependencyStore.each(function(dependency) {
                ...
                if (from === task && to) {
                    task.successors.push(dependency);
                    ...
                    onLinkEstablished && onLinkEstablished(from, to);
                }

                if (to === task && from) {
                    task.predecessors.push(dependency);
                    if (!to.successors) {
                        to.successors = [];
                    }
                    from.successors.push(dependency);
    ...
    onMyNodeAdded : function (parent, node) {
        ...
        if (!node.isRoot()) {
            ...
            // Corresponding logic is present in DependencyStore::onDependencyAdded
            node.cascadeBy(function(nodeOrDescendant) {
                me.fillSingleTaskWithDepInfo(nodeOrDescendant, function onMyNodeAddedLinkEstablished(from, to) {
                    // ensure that tasks being linked are fully scheduled only if we are not currently undoing or redoing
                    if (!me.isUndoingOrRedoing() && me.getDependencyStore()) {
                        me.getDependencyStore().scheduleLinkedTasks(from, to);
                    }
                });
            });
...
//
    ...
    indent: function (nodes, callback) {
        ...
        me.suspendEvents('beforeindentationchange', 'indentationchange');
        ...
        (function processStep() {
            if (nodesToProcess.length) {
                ...
                me.resumeEvents('beforeindentationchange', 'indentationchange');
    ...
    outdent: function (nodes, callback) {
        ...
        // 4. Outdanting taking constraints related behaviour in mind
        me.beginIndent(nodes, context);
        me.suspendEvents('beforeindentationchange', 'indentationchange');
