...
Ext.define('Sch.mixin.Zoomable', {
    ...
    */
    ...
    zoomToLevel: function (level, span, options) {
        ...
        // we want to get 'scroll' event after calling view.scroll*
        // in some cases coordinate could remain unchanged and scroll event wouldn't be thrown
        // if we call this method twice scroll event is fired for sure and only once
        if (isVertical) {
            var y = view.getYFromDate(centerDate, true) - panelSize / 2;
            ...
            scrollSource.on('scroll', function () {
                ...
                // Scrolling method, called outside of this listener is not last scroll performed, there's also 
                // headerCt scroll happening that messes the result. That scroll is async, and we don't know 
                // where it comes from (scrollTask maybe?). Ideally, we need to disable saving/restoring scroll 
                // before switchViewPreset is called. We also tested suspendLayout/resumeLayout(true) around 
                // method's body, overriding restoreScrollState on locked grid, defering scrolling, none work simple 
                // enough and require further investigation (tests are failing).
                // Applied solution, on the other hand, solves our zooming tests with just increased { waitFor }.
                view.scrollVerticallyTo(y);
            }, this, { single : true });            
            
            view.scrollVerticallyTo(y - 1);
            view.scrollVerticallyTo(y);
        } else {
            var x = view.getXFromDate(centerDate, true) - panelSize / 2;
            
            scrollSource.on('scroll', function () {
                me.cachedCenterDate = centerDate; 
                view.scrollHorizontallyTo(x);
            }, this, { single : true });
            
            view.scrollHorizontallyTo(x - 1);
            view.scrollHorizontallyTo(x);
        }
