...
Ext.define('Gnt.widget.taskeditor.TaskEditor', {
    ...
    loadTask : function (task) {
        ...
        if (this.notesEditor) {
            this.notesEditor.setValue(task.getNote());
            this.notesEditor.setReadOnly(!task.isEditable(task.noteField));
        }
    ...
    doValidate : function(invalidComponentsAccFn) {
        ...
        if (this.taskForm && !this.taskForm.isValid()) {
            result = false;
            invalidComponentsAccFn && invalidComponentsAccFn(this.getTabByComponent(this.taskForm));
        }

        if (this.dependencyGrid && !this.dependencyGrid.isValid()) {
            result = false;
            invalidComponentsAccFn && invalidComponentsAccFn(this.getTabByComponent(this.dependencyGrid));
        }

        if (this.assignmentGrid && !this.assignmentGrid.isValid()) {
            result = false;
            invalidComponentsAccFn && invalidComponentsAccFn(this.getTabByComponent(this.assignmentGrid));
        }

        if (this.advancedForm && !this.advancedForm.isValid()) {
            result = false;
            invalidComponentsAccFn && invalidComponentsAccFn(this.getTabByComponent(this.advancedForm));
        }
    ...
    updateTask : function () {
        ...
        if (me.fireEvent('beforeupdatetask', me, function() {me.doUpdateTask();}) !== false) {
            me.doUpdateTask();
            me.fireEvent('afterupdatetask', me);
            result = true;
        }
    ...
    isDataValid : function(invalidComponentsAccFn) {
